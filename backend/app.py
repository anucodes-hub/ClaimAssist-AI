...
from models.database import db  # <--- NEW: Import the db object
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///claimassist.db'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

# 4. Initialize Database and Middleware
db.init_app(app) # <--- NEW: Link SQLAlchemy to your Flask app
CORS(app, origins=["http://localhost:5173"])
jwt = JWTManager(app)
os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)

# 5. Chatbot Route (Remains the same)
...
if __name__ == '__main__':
    with app.app_context():
        db.create_all()  # <--- This handles table creation for ALL models
